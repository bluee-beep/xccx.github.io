...
- **首屏与导航**：修复触屏设备 Hero Logo 使用 `fixed` 后贯穿全页的问题；移动端 Header 改为固定双行布局，保留全部章节锚点并支持横向滚动、激活项自动居中，同时补齐刘海屏安全区
- **内容排版**：≤768px 时章节正文由 60vw 窄栏恢复为容器全宽，统一左对齐并收紧字号、留白、背景水印与波浪线，避免小屏断行过密
- **触控交互**：联系方式改为可聚焦语义元素；GitHub/邮箱可直接打开，触屏常显账号值，微信支持点击展开自适应宽度二维码
- **性能与过渡**：移动端章节圆弧舞台降级为静态渐变，页尾拖影仅保留主层；动态正文取消无效 `will-change`，条带覆盖改为容器宽度，避免 `100vw` 横向溢出
- **开发体验**：CursorGrid 诊断信息仅在显式传入 `?cursor=` 时显示，不再遮挡移动端调试画面
